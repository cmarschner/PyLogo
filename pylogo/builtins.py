...
def logo_repr(arg):
    if isinstance(arg, list):
        return '[%s]' % _join(list(map(logo_soft_repr, arg)))
...
def pr(*args):
    ...
    for arg in args:
        if isinstance(arg, list):
            trans.append(_join(list(map(logo_soft_repr, arg))))
        else:
...
def _to_degrees(num):
    return (old_div(num, math.pi)) * 180
def _to_radians(num):
...
def procedures(interp):
    ...
    in the workspace.  
    """
    return list(interp.functions.keys())
